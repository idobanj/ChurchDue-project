...
export async function recordPayment(
  ...
  const { reference, dueId, studentId, paystackSecretKey, executionId } = params
  ...
  if (dueError) {
    ...
    return { success: false, status: 500, error: 'Database error while fetching due' }
  }
  if (!dueData) {
    return { success: false, status: 404, error: 'Due not found' }
  }
  if (dueData.status !== 'active') {
    return { success: false, status: 400, error: 'This due is not accepting payments' }
  }

  // 2. Fetch student profile and check role/organization match
  const { data: studentProfile, error: studentProfileError } = await adminSupabase
    .from('users')
  ...
  if (studentProfile.role !== 'student' || studentProfile.organization_id !== dueData.organization_id) {
  ...
  if (!paystackData.status || paystackData.data?.status !== 'success') {
    return { success: false, status: 400, error: 'Payment verification failed' }
  }
  ...
  if (metadataDueId !== dueId || metadataStudentId !== studentId) {
    return { success: false, status: 403, error: 'Payment metadata mismatch' }
  }
  ...
  if (existingPaymentError) {
    ...
    return { success: false, status: 500, error: 'Database error while checking payment' }
  }
  ...
  if (existingPayment) {
    if (
      ...
      return { success: false, status: 409, error: 'Payment reference already belongs to another record' }
    }
  ...
  if (previousPaymentsError) {
    ...
    return { success: false, status: 500, error: 'Database error while checking balance' }
  }
  ...
  if (amountInNaira <= 0 || amountInNaira > remainingAmount) {
    return { success: false, status: 400, error: 'Payment amount exceeds outstanding balance' }
  }
  ...
  if (paymentError) {
    if (paymentError.code === '23505') {
      return { success: false, status: 409, error: 'Payment has already been recorded' }
    }
