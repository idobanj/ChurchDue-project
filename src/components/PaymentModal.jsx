...
export default function PaymentModal({ due, onClose }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Define the callback as a standard variable
      const handlePaystackSuccess = async (response) => {
        try {
          const { error: funcError } = await supabase.functions.invoke('verify-paystack-payment', {
            body: {
      ...
      // Call setup using the specific Paystack namespace
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-2">
            ...
            <button type="submit" className="flex-1 p-2 bg-blue-600 text-white rounded">Pay Now</button>
          </div>
        </form>
      </div>
    </div>
